...
function SelectFilter({ label, value, onChange, options, emptyLabel }) {
  return (
    <label className="min-w-0 flex-1">
      ...
      <select
        ...
        aria-label={label}
        className="w-full rounded-lg border border-ink-border bg-white px-3 py-2.5 text-sm font-semibold text-ink-primary outline-none focus:border-brand focus:ring-2 focus:ring-brand/20"
      >
...
export default function SensorFilters({ stations, filters, onChange }) {
  ...
  const provinces = unique(stations.map((station) => station.province));
  const districts = unique(provinceStations.map((station) => station.district));
  ...
  return (
    <section className="mb-8 rounded-xl border border-ink-border bg-white p-4 shadow-sm">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <SelectFilter label="District" value={filters.district} onChange={(value) => onChange("district", value)} options={districts} emptyLabel="No districts available" />

...
export default function ManageUsers() {
  ...
  function openEdit(user) {
    ...
    setFormData({
      ...
      email_alerts: Boolean(user.email_alerts),
    });
    setModal("edit");
  }
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (modal === "create") {
        ...
        replaceUser(await updateUser(selectedUser.id, payload));
        setMessage("User details updated successfully.");
      }
      closeModal();
    } catch (err) {
      setError(err.response?.data?.detail || "Could not save the user.");
    } finally {
      setIsSaving(false);
    }
  }
  ...
  async function handleDelete(user) {
    if (!window.confirm(`Delete ${user.name}? Accounts with historical reports cannot be deleted.`)) return;
    ...
    try {
      ...
      setError(err.response?.data?.detail || "Could not delete the user.");
    } finally {
      setWorkingId(null);
    }
  }
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" role="dialog" aria-modal="true">
          <div className="max-h-[90vh] w-full max-w-xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
            ...
            {(modal === "create" || modal === "edit") && <form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2"><label className="text-sm font-semibold text-ink-primary">Full name<input required minLength="2" value={formData.name} onChange={(event) => updateField("name", event.target.value)} className="mt-2 w-full rounded-lg border border-ink-border px-4 py-3 outline-none focus:border-brand focus:ring-2 focus:ring-brand/20" /></label><label className="text-sm font-semibold text-ink-primary">Email<input required type="email" value={formData.email} onChange={(event) => updateField("email", event.target.value)} className="mt-2 w-full rounded-lg border border-ink-border px-4 py-3 outline-none focus:border-brand focus:ring-2 focus:ring-brand/20" /></label><label className="text-sm font-semibold text-ink-primary">Phone<input value={formData.phone} onChange={(event) => updateField("phone", event.target.value)} className="mt-2 w-full rounded-lg border border-ink-border px-4 py-3 outline-none focus:border-brand focus:ring-2 focus:ring-brand/20" /></label><label className="text-sm font-semibold text-ink-primary">District<input value={formData.district} onChange={(event) => updateField("district", event.target.value)} className="mt-2 w-full rounded-lg border border-ink-border px-4 py-3 outline-none focus:border-brand focus:ring-2 focus:ring-brand/20" /></label>{modal === "create" && <><label className="text-sm font-semibold text-ink-primary">Initial password<input required type="password" minLength="8" value={formData.password} onChange={(event) => updateField("password", event.target.value)} className="mt-2 w-full rounded-lg border border-ink-border px-4 py-3 outline-none focus:border-brand focus:ring-2 focus:ring-brand/20" /></label><label className="text-sm font-semibold text-ink-primary">Role<select value={formData.role} onChange={(event) => updateField("role", event.target.value)} className="mt-2 w-full rounded-lg border border-ink-border bg-white px-4 py-3 capitalize outline-none focus:border-brand focus:ring-2 focus:ring-brand/20">{roles.map((role) => <option key={role} value={role}>{roleLabel(role)}</option>)}</select></label></>}<label className="flex items-center gap-3 rounded-lg border border-ink-border p-4 text-sm font-semibold text-ink-primary sm:col-span-2"><input type="checkbox" checked={formData.email_alerts} onChange={(event) => updateField("email_alerts", event.target.checked)} className="h-4 w-4" />Enable email alerts (SNS confirmation required)</label><button disabled={isSaving} className="rounded-lg bg-brand px-4 py-3 font-bold text-white sm:col-span-2 disabled:opacity-60">{isSaving ? "Saving..." : modal === "create" ? "Create user" : "Save changes"}</button></form>}
          </div>
        </div>
      )}
    </AdminLayout>

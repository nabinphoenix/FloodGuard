        ...
        {isLoading ? <LoadingSpinner message="Checking sensor systems..." /> : (
          <>
            <div className="grid gap-5 md:grid-cols-2">
              ...
              <article className="rounded-2xl border border-blue-100 bg-white p-6 shadow-sm"><div className="flex items-start justify-between"><div><p className="text-sm font-bold uppercase tracking-wide text-slate-500">Latest Sensor Reading</p><p className="mt-4 text-lg font-black text-blue-950">{health?.latest_sensor_reading ? Number(health.latest_sensor_reading.water_level).toFixed(2) + " m" : "No readings found"}</p><p className="mt-2 text-xs text-slate-500">{health?.latest_sensor_reading ? formatKathmanduDateTime(health.latest_sensor_reading.timestamp) : "No sensor readings are available yet."}</p></div><Waves className="text-brand" size={26} /></div></article>
            </div>
            <div className="mt-6 rounded-xl border border-blue-100 bg-blue-50 p-5 text-sm text-blue-900"><p><strong>Last ingestion time:</strong> {health?.last_sensor_reading_time ? formatKathmanduDateTime(health.last_sensor_reading_time) : "No readings found"}</p><p className="mt-2">Sensor telemetry is saved before notification delivery is attempted. A failed SNS or SQS call does not remove a saved reading.</p></div>
          </>
        )}
      </section>
    </AdminLayout>

...
function statusClass(status) {
  return { safe: "bg-green-100 text-green-800", watch: "bg-yellow-100 text-yellow-800", warning: "bg-orange-100 text-orange-800", emergency: "bg-red-100 text-red-800", no_data: "bg-slate-100 text-slate-700" }[status] || "bg-slate-100 text-slate-700";
}
...
export default function SensorDash() {
  ...
  useEffect(() => {
    load();
    const timer = window.setInterval(load, DASHBOARD_REFRESH_INTERVAL_MS);
  ...
  const latestTelemetry = recent[0] || null;

  return (
    <AdminLayout title="Sensor Dashboard">
      <section>
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div><h1 className="text-3xl font-black tracking-tight text-ink-primary">Field Officer Dashboard</h1><p className="mt-2 text-ink-secondary">Operational sensor monitoring and recent station telemetry.</p>{lastUpdated && <p className="mt-1 text-xs text-ink-secondary">Last updated: {lastUpdated.toLocaleTimeString()}</p>}</div>
          <div className="flex flex-wrap gap-3"><Link to="/sensors/stations?create=1" className="flex items-center gap-2 rounded-lg bg-brand px-4 py-3 font-bold text-white hover:bg-brand-gradientEnd"><Plus size={17} /> Add Sensor Station</Link><button type="button" onClick={() => load(true)} disabled={isRefreshing} className="flex items-center gap-2 rounded-lg border border-ink-border bg-white px-4 py-3 font-semibold shadow-sm hover:border-brand hover:text-brand disabled:opacity-60"><RefreshCw size={17} className={isRefreshing ? "animate-spin" : ""} /> {isRefreshing ? "Refreshing..." : "Refresh"}</button></div>
        </div>

        <aside className="mb-6 rounded-2xl border border-sky-200 bg-sky-50 p-5 text-sm text-sky-950">
          <h2 className="font-black">Automatic Sensor Monitoring</h2>
          <p className="mt-1">Recent sensor readings are received through the FloodGuard cloud sensor pipeline.</p>
          {latestTelemetry ? (
            <div className="mt-3 flex flex-wrap items-center gap-x-4 gap-y-2 text-xs font-semibold">
              <span>Telemetry: Active</span>
              <span>Last reading: {formatReadingAge(latestTelemetry.timestamp)}</span>
              <span className={`rounded-full px-2 py-1 ${freshnessClass(latestTelemetry.freshness)}`}>Freshness: {freshnessLabel(latestTelemetry.freshness)}</span>
            </div>
          ) : (
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="mt-6 grid gap-6 xl:grid-cols-[1.4fr_1fr]">
              ...
              <article className="rounded-2xl border border-blue-100 bg-white p-6 shadow-sm"><h2 className="text-xl font-black text-blue-950">Recent Sensor Readings</h2><p className="mt-1 text-sm text-slate-600">Most recent telemetry received by the API.</p><div className="mt-5 overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b text-xs uppercase tracking-wide text-slate-500"><tr><th className="pb-2">Station</th><th className="pb-2">Level</th><th className="pb-2">State</th></tr></thead><tbody className="divide-y divide-slate-100">{recent.length === 0 ? <tr><td colSpan="3" className="py-4 text-slate-600">No sensor readings are available yet.</td></tr> : recent.map((reading) => <tr key={`${reading.station_code}-${reading.timestamp}`}><td className="py-3"><p className="font-bold text-blue-950">{reading.station_code}</p><p className="text-xs text-slate-500">{formatReadingAge(reading.timestamp)}</p></td><td className="py-3 font-bold">{Number(reading.water_level).toFixed(2)} m</td><td className="py-3"><span className={`mr-1 rounded-full px-2 py-1 text-[11px] font-bold ${statusClass(reading.status)}`}>{statusLabel(reading.status)}</span><span className={`rounded-full px-2 py-1 text-[11px] font-bold ${freshnessClass(reading.freshness)}`}>{freshnessLabel(reading.freshness)}</span></td></tr>)}</tbody></table></div></article>
            </div>
          </>
        )}
      </section>

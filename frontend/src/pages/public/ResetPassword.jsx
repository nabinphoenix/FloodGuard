  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-blue-50 px-4 py-12">
      <section className="w-full max-w-md rounded-lg border border-blue-100 bg-white p-8 shadow-xl shadow-blue-100">
        ...
        {message ? <div className="mt-6"><FeedbackMessage message={<>{message} <Link to="/login" className="font-bold underline">Sign in</Link></>} type="success" /></div> : null}
        {!message ? <div className="mt-6"><FeedbackMessage message={error} onDismiss={() => setError("")} /></div> : null}
        {!message && <form className="mt-7 space-y-5" onSubmit={handleSubmit}>
          ...
          <Button type="submit" disabled={!token} isLoading={isSubmitting} loadingLabel="Updating..." className="w-full rounded-md bg-blue-700 px-4 py-3 text-base hover:bg-blue-800">Update password</Button>
        </form>}
